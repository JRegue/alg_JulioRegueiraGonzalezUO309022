...
public class NCaballos {
    ...
    private boolean[][] tableroPosible;
     ...
     */
    public List<List<String>> resolverNCaballos(int n) {
        soluciones = new ArrayList<>();
        
        // Inicializamos los rastreadores de ataques en columnas y diagonales
        tableroPosible = new boolean[n][n];
    ...
    private void backtracking(int fila, int n, char[][] tablero) {
        ...
        for (int col = 0; col < n; col++) {
            ...
            //por qué coño no va
            if (!tableroPosible[fila][col]) {
                if (fila + 2 < n) {
                    if(col - 1 >= 0){
                        tableroPosible[fila + 2][col-1] = true;
                    }
                    if(col + 1 < n){
                        tableroPosible[fila + 2][col + 1] = true;
                    }
                ...
                if (col + 2 < n) {
                    if(fila - 1 >= 0){
                        tableroPosible[fila -1][col + 2] = true;
                    }
                    if(fila + 1 < n){
                        tableroPosible[fila + 1][col + 2] = true;
                    }
            ...
            // 3. Retroceder
                if (fila + 2 < n) {
                    if(col - 1 >= 0){
                        tableroPosible[fila + 2][col-1] = false;
                    }
                    if(col + 1 < n){
                        tableroPosible[fila + 2][col + 1] = false;
                    }
                ...
                if (col + 2 < n) {
                    if(fila - 1 >= 0){
                        tableroPosible[fila -1][col + 2] = false;
                    }
                    if(fila + 1 < n){
                        tableroPosible[fila + 1][col + 2] = false;
                    }
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        List<List<String>> resultado = algoritmo.resolverNCaballos(n);
